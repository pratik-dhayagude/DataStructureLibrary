...
template<class T>
int  SinglyLLL<T>:: Summation(T no)
{
    int Sum = 0;
    while(this->first != NULL)
    {
        iSum = iSum + this -> first -> data;
        this -> first = this->first -> next;
...
template<class T>
int  SinglyLLL<T>:: Maximum(T no)
{
    int Max = this->first->data;
    while(this->first != NULL)
    {
        if(this->first->data > Max)
        {
            Max = this -> first -> data;
        }
        this ->first = this->first->next;
...
template<class T>
int  SinglyLLL<T>:: Minimum(T no)
{
    int Min = this -> first -> data;

    while(this->first != NULL)
    {
        if(this->first->data < Min)
        {
            Min = this->first->data;
        }
        this->first = this->first->next;
